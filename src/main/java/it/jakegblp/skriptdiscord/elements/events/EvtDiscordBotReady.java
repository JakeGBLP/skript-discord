...
public class EvtDiscordBotReady extends SkriptEvent {

    static {
        Skript.registerEvent("Discord - on Bot Ready", EvtDiscordBotReady.class, BukkitReadyEvent.class, "[skript( |-)]discord bot[s] [%-*strings%] read(y|ied)")
                .description("""
                        Called when a discord bot is ready.
                        """)
                .examples("on discord bot ready:")
                .since("1.0.0");
    }
    ...
    public boolean check(Event event) {
        ...
        Bot bot = ((BukkitReadyEvent) event).getBot();
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "skript-discord bot "+nameLiteral.toString(event, debug)+" ready";
    }
}
